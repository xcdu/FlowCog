...
class TextCNNRNN(object):
  def __init__(self, embedding_mat, non_static, hidden_unit, sequence_length,
    ...
    lstm_cell1 = tf.contrib.rnn.GRUCell(num_units=hidden_unit)
    lstm_cell2 = tf.contrib.rnn.GRUCell(num_units=hidden_unit)
    stack = tf.contrib.rnn.MultiRNNCell([lstm_cell1, lstm_cell2])

    lstm_cell = tf.contrib.rnn.DropoutWrapper(stack,
    ...
    with tf.name_scope('output'):
      self.W = tf.Variable(
          tf.truncated_normal([hidden_unit, num_classes], stddev=0.1), name='W')
      b = tf.Variable(tf.constant(0.1, shape=[num_classes]), name='b')
      l2_loss += tf.nn.l2_loss(W)
      l2_loss += tf.nn.l2_loss(b)
      self.scores = tf.nn.xw_plus_b(output, self.W, b, name='scores')
      self.predictions = tf.argmax(self.scores, 1, name='predictions')
      # print_out = tf.Print(self.predictions, [self.predictions])
