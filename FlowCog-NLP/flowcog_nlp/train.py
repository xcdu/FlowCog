...
def train_cnn_rnn():
  ...
  x_, y_, vocabulary, vocabulary_inv, df, labels = helper.load(
      input_file)

  training_config = sys.argv[2]
  params = json.loads(open(training_config).read())

  word_embeddings = helper.init_embedding(vocabulary)
  ...
  logging.info(
      'x_train: {}, x_dev: {}, x_test: {}'.format(len(x_train), len(x_dev),
                                                  len(x_test)))
  logging.info(
      'y_train: {}, y_dev: {}, y_test: {}'.format(len(y_train), len(y_dev),
                                                  len(y_test)))

  # Create a directory, everything related to the training will be saved in
  #  this directory
  timestamp = str(int(time.time()))
  trained_dir = 'models/trained_results_' + timestamp + '/'
    ...
    with sess.as_default():
      ...
      def dev_step(x_batch, y_batch):
        ...
        step, loss, accuracy, num_correct, predictions = sess.run(
            [global_step, cnn_rnn.loss, cnn_rnn.accuracy, cnn_rnn.num_correct,
             cnn_rnn.predictions], feed_dict)
      ...
      train_batches = helper.batch_iter(list(zip(x_train, y_train)),
                                        params['batch_size'],
                                        params['num_epochs'])
      ...
      for train_batch in train_batches:
        ...
        if current_step % params['evaluate_every'] == 0:
          dev_batches = helper.batch_iter(list(zip(x_dev, y_dev)),
                                          params['batch_size'], 1)

          total_dev_correct = 0
          for dev_batch in dev_batches:
            x_dev_batch, y_dev_batch = zip(*dev_batch)
            acc, loss, num_dev_correct, predictions = dev_step(x_dev_batch,
                                                               y_dev_batch)
          ...
          if accuracy >= best_accuracy:
            ...
            logging.critical(
                'Best accuracy {} at step {}'.format(best_accuracy,
                                                     best_at_step))
      logging.critical(
      ...
      test_batches = helper.batch_iter(list(zip(x_test, y_test)),
                                       params['batch_size'], 1,
                                       shuffle=False)
      total_test_correct = 0
      for test_batch in test_batches:
        x_test_batch, y_test_batch = zip(*test_batch)
        acc, loss, num_test_correct, predictions = dev_step(x_test_batch,
                                                            y_test_batch)
...
if __name__ == '__main__':
  # python3 train.py ./data/gathered.over.pd ./training_config.json
  train_cnn_rnn()
