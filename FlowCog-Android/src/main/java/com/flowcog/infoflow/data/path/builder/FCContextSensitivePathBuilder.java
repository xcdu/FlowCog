...
public class FCContextSensitivePathBuilder extends FCConcurrentAbstractionPathBuilder {
  private static int sourceFindingTaskNum = 0;

  protected ConcurrentIdentityHashMultiMap<Abstraction, SourceContextAndPath> pathCache = new ConcurrentIdentityHashMultiMap<>();

  /**
   * Creates a new instance of the {@link FCContextSensitivePathBuilder} class
   *
   * @param manager
   ...
   */
  public FCContextSensitivePathBuilder(InfoflowManager manager, InterruptableExecutor executor) {
  ...
  protected class SourceFindingTask implements Runnable {
    ...
    public SourceFindingTask(Abstraction abstraction) {
      this.taskNum = sourceFindingTaskNum++;
      this.abstraction = abstraction;
    }

    public SourceFindingTask(Abstraction abstraction, int parent) {
      this.taskNum = sourceFindingTaskNum++;
      this.abstraction = abstraction;
      ...
      if (pred != null && paths != null) {
        for (SourceContextAndPath scap : paths) {
          ...
          if (processPredecessor(scap, pred))
            ...
            scheduleDependentTask(new SourceFindingTask(pred, this.taskNum));
          ...
          if (pred.getNeighbors() != null)
            for (Abstraction neighbor : pred.getNeighbors())
              if (processPredecessor(scap, neighbor))
                // Schedule the predecessor
                scheduleDependentTask(new SourceFindingTask(neighbor, this.taskNum));
        }
      }
    }
